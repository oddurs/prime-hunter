//! Volunteer public API — v1 REST endpoints for the volunteer computing platform.
//!
//! Implements the public work API that volunteers interact with:
//! registration, worker management, work claiming, result submission,
//! personal stats, and the leaderboard.
//!
//! All endpoints except `/api/v1/register` and `/api/v1/leaderboard` require
...
async fn authenticate(
    state: &Arc<AppState>,
    headers: &HeaderMap,
) -> Result<VolunteerRow, (StatusCode, Json<serde_json::Value>)> {
    let auth = headers
        .get("authorization")
    ...
    match state.db.get_volunteer_by_api_key(api_key).await {
        Ok(Some(vol)) => {
            // Update last_seen
            let _ = state.db.touch_volunteer(vol.id).await;
            Ok(vol)
        }
        Ok(None) => Err((
            StatusCode::UNAUTHORIZED,
    ...
    match state.db.claim_volunteer_block(vol.id, &caps).await {
        Ok(Some(block)) => {
            ...
            if let Some(ref search_type) = block.search_type {
                let trust = state.db.get_volunteer_trust(vol.id).await.ok().flatten();
...
pub(super) async fn handler_v1_result(
    ...
    if let Err(e) = state
        .db
        .submit_volunteer_result(payload.block_id, payload.tested, payload.found)
        .await
    {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    for prime in &payload.primes {
        ...
        {
            Ok(_) => {
                let _ = state.db.increment_volunteer_primes(vol.id).await;
            ...
            Err(e) => {
                eprintln!(
                    "Warning: failed to insert volunteer prime {}: {}",
                    prime.expression, e
                );
            }
        }
...
pub(super) async fn handler_v1_stats(
    ...
    match state.db.get_volunteer_stats(vol.id).await {
        Ok(Some(stats)) => (
            StatusCode::OK,
            Json(serde_json::json!({
...
pub(super) async fn handler_v1_leaderboard(
    ...
    match state.db.get_volunteer_leaderboard(100).await {
        Ok(entries) => {
